...
export function AdminQuotationClientPanel({ initialLeads, initialQuotations, selectedLeadId }: AdminQuotationPanelProps) {
  ...
  async function handleCreateQuotation(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setFeedback({ type: "error", message: "Unable to create quotation. Please try again." });
    } finally {
      setSubmitting(false);
    }
  }
  ...
  async function handleStatusChange(id: string, status: QuotationStatus) {
    ...
    try {
      ...
      setQuotations(previous);
      setFeedback({ type: "error", message: "Unable to update quotation status. Please try again." });
    } finally {
      setUpdatingId(null);
    }
  }
  ...
  async function handleCopyShareUrl(token: string) {
    ...
    try {
      ...
      console.error("Share quote link copy failed", error);
      setFeedback({ type: "error", message: "Unable to copy link. Open the share link and copy it from the address bar." });
    }
  }
  ...
  async function handleSendEmail(id: string) {
    ...
    try {
      ...
      setFeedback({ type: "error", message: "Unable to send quotation email. Please try again." });
    } finally {
      setEmailingId(null);
    }
  }
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-[420px_minmax(0,1fr)]">
      <form onSubmit={handleCreateQuotation} className="grid gap-4 rounded-lg border border-brand-border bg-brand-card p-5">
        <h2 className="text-xl font-semibold">Create Customer Quote</h2>
      ...
      <div className="space-y-4">
        ...
        {quotations.length === 0 ? (
          <div className="rounded-lg border border-brand-border bg-brand-card p-5 text-sm text-brand-muted">
            No quotations created yet. Start from a lead, build a draft, then send a secure quote link to the customer.
          </div>
        ) : (
              ...
              <table className="w-full min-w-[1280px] text-left text-sm">
                ...
                <tbody className="divide-y divide-brand-border">
                  {quotations.map((quote) => {
                    const publicShareToken = quote.publicShareToken;
                    ...
                    <tr key={quote.id}>
                        ...
                        {publicShareToken ? (
                          <div className="flex flex-col gap-2">
                            <Link
                              ...
                              href={`/quote/share/${publicShareToken}`}
                            >
                              View Share Link
                            </Link>
                            <button
                              className="inline-flex rounded-md border border-brand-border px-3 py-2 text-xs font-semibold text-brand-text transition hover:border-brand-accent hover:text-brand-accent"
                              onClick={() => void handleCopyShareUrl(publicShareToken)}
                              type="button"
                            >
                              Copy URL
                            </button>
                      ...
                      <td className="px-4 py-4 align-top">
                        <Link
                          ...
                          href={
                            publicShareToken
                              ? `/api/public/quotes/${publicShareToken}/pdf`
